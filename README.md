...
* Angular 8 ou 9  (https://loiane.training/curso/angular  - Grátis) 
...
2. Seguir o passo a paso do https://po-ui.io/guides/getting-started
  ...
  b. ng new my-po-project --skipInstall
    ...
    ```
    c. ```npm install```
    d. ```ng add @po-ui/ng-components```
    e. ```npm i --save @po-ui/ng-templates```
    f. E depois adicionar o PoTemplatesModule no módulo principal (app.module.ts) da sua aplicação 
    g. ```ng serve```
    h. Utilizando o tema da TOTVS no portinari (https://thf.totvs.com.br/home) -> ```npm i --save @totvs/po-theme```
